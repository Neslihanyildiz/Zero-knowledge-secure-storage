...
// DELETE /api/files/:fileId  (protected — owner only)
exports.deleteFile = async (req, res) => {
    try {
        const { fileId } = req.params;

        // IDOR check: only the owner may delete
        ...
        if (fetchError || !file) return res.status(404).json({ error: 'Dosya bulunamadı.' });
        if (file.user_id !== req.user.id) return res.status(403).json({ error: 'Bu dosyayı silme yetkiniz yok.' });

        // Remove all share records first, then the file row, then storage blob
        ...
        await supabase.from('activity_logs').insert({
            user_id: req.user.id,
            action: 'DELETE',
            details: `${req.user.username} dosyayı sildi: ${file.filename}`,
            ip_address: req.ip,
        });

        res.json({ message: 'Dosya silindi.' });
    } catch (error) {
        res.status(500).json({ error: 'Silme hatası.', details: error.message });
    }
};
