...
/**
 * Re-wrap the RSA private key under a new AES-GCM derived key.
 * Decrypts the old IV+ciphertext blob, immediately re-encrypts the raw bytes
 * with a fresh IV and the new key. The JWK bytes exist only as a transient
 * ArrayBuffer — they are never parsed, exported, or returned.
 */
export async function rewrapPrivateKey(
  ...
  const newCombined = new Uint8Array(12 + newCiphertext.byteLength);
  newCombined.set(newIv, 0);
  newCombined.set(new Uint8Array(newCiphertext), 12);
