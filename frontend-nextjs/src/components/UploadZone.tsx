...
import { generateAESKey, encryptFile, encryptString } from "@/lib/aes"; // ← encryptString eklendi
...
export default function UploadZone({ onUploadSuccess, user }: UploadZoneProps) {
  ...
  const handleUpload = async (file: File) => {
    ...
    try {
      ...
      // 6b. Dosya adını aynı AES key ile şifrele → sunucu düz adı hiç görmez
      const encryptedFilename = await encryptString(file.name, aesKey);
      ...
      //    - 'originalFilename' → AES-GCM ile şifrelenmiş dosya adı (base64)
      ...
      formData.append("originalFilename", encryptedFilename); // ← file.name yerine encryptedFilename
