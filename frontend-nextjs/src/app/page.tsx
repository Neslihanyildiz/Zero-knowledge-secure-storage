...
export default function AuthPage() {
  ...
  const { setUser, setEncryptedKey } = useAuthStore();
  ...
  const handleSubmit = async (e: { preventDefault(): void }) => {
    ...
    try {
      if (mode === "register") {
        ...
        // No keys on record — admin reset this user's password. Generate a fresh
        // RSA key pair, wrap the private key with the new password, and upload to
        // the server so future logins on any device can restore them.
        let finalEncryptedKey = res.encrypted_private_key ?? null;
        let finalKeySalt      = res.key_salt ?? null;

        if (!keyRestored && !res.encrypted_private_key) {
          try {
            const keyPair        = await generateRSAKeyPair();
            const publicKeyStr   = await exportKey(keyPair.publicKey);
            const privateKeyStr  = await exportKey(keyPair.privateKey);
            ...
            const encPrivKey     = await wrapPrivateKey(keyPair.privateKey, wrappingKey);
            const keySalt        = saltToBase64(salt);
            await api.updateKeys(publicKeyStr, encPrivKey, keySalt);
            await storePrivateKey(res.user.username, privateKeyStr);
            localStorage.setItem(`pub_${res.user.username}`, publicKeyStr);
            res.user.public_key  = publicKeyStr;
            finalEncryptedKey    = encPrivKey;
            finalKeySalt         = keySalt;
          } catch (keyErr) {
            console.warn("Key generation after password reset failed:", keyErr);
          }
        }
        ...
        setEncryptedKey(finalEncryptedKey, finalKeySalt);
        ...
        <div className="card p-8">
          ...
          <div className="flex bg-navy-50 rounded-xl p-1 mb-8">
            {(["login", "register"] as Mode[]).map((m) => (
              <button
                key={m}
                onClick={() => { setMode(m); setError(""); setPassword(""); setShowForgot(false); }}
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            {mode === "login" && showForgot && (
              <div className="px-4 py-4 rounded-xl bg-navy-50 border border-navy-200 space-y-2">
                <p className="text-sm font-semibold text-navy-800">How to recover your account</p>
                <ol className="text-xs text-navy-600 space-y-1.5 list-decimal list-inside">
                  <li>Ask your <strong>system administrator</strong> to reset your password.</li>
                  <li>Log in with the temporary password they provide.</li>
                  <li>New encryption keys will be generated automatically on first login.</li>
                  <li>Go to <strong>Settings → Change Password</strong> to set a permanent password.</li>
                </ol>
                <p className="text-xs text-navy-500 pt-1">
                  Note: files encrypted under your old key cannot be recovered — this is a
                  property of zero-knowledge encryption.
                </p>
                <button
                  type="button"
